		...
		<div class="container">
			<div class="row mb-20">
				...
				<div class="col-md-6 text-right">
					<a href="<?php echo base_url('funding/add'); ?>" class="btn btn-info mb-0 mblock-btn">Request For Funding</a>
			...
			<div class="row">
				<div class="col-md-3">
					...
					if($_GET['investment_currency']=='ZAR'){ echo "selected";}} ?> value="ZAR">South African Rand </option>
							...
							<div class="row">
								<div class="col-xs-6">
									<input type="text" name="min_invest" class="form-control" value="<?php if(!empty($_GET['min_invest'])){ echo $_GET['min_invest']; }  ?>"  placeholder="min" />
								</div>
								<div class="col-xs-6">
									<input type="text" name="max_invest" class="form-control" value="<?php if(!empty($_GET['max_invest'])){ echo $_GET['max_invest']; }  ?>"  placeholder="max" />
								</div>
							</div>

					<label>Share
					<select name="share_currency" class="form-control sub-dw pull-right">
					<option value="">Select Currency</option>			
					<option <?php if(!empty($_GET['share_currency'])){ 
					if($_GET['share_currency']=='INR'){ echo "selected";}} ?>  value="INR">India</option>
					<option <?php if(!empty($_GET['share_currency'])){ 
					if($_GET['share_currency']=='USD'){ echo "selected";}} ?> value="USD">U.S. Dollar</option>
					<option <?php if(!empty($_GET['share_currency'])){ 
					if($_GET['share_currency']=='EUR'){ echo "selected";}} ?> value="EUR">European Euro</option>
					<option <?php if(!empty($_GET['share_currency'])){ 
					if($_GET['share_currency']=='JPY'){ echo "selected";}} ?> value="JPY">Japanese </option>
					<option <?php if(!empty($_GET['share_currency'])){ 
					if($_GET['share_currency']=='GBP'){ echo "selected";}} ?> value="GBP">British Pound </option>
					<option <?php if(!empty($_GET['share_currency'])){ 
					if($_GET['share_currency']=='CHF'){ echo "selected";}} ?> value="CHF">Swiss Franc </option>
					<option <?php if(!empty($_GET['share_currency'])){ 
					if($_GET['share_currency']=='CAD'){ echo "selected";}} ?> value="CAD">Canadian Dollar </option>
					<option <?php if(!empty($_GET['share_currency'])){ 
					if($_GET['share_currency']=='AUD'){ echo "selected";}} ?> value="AUD">Australian </option>
					<option <?php if(!empty($_GET['share_currency'])){ 
					if($_GET['share_currency']=='ZAR'){ echo "selected";}} ?> value="ZAR">South African Rand </option>
								</select>
								
							</label>
							<div class="row">
								<div class="col-xs-6">
									<input type="text" name="min_share" class="form-control" value="<?php if(!empty($_GET['min_share'])){ echo $_GET['min_share']; }  ?>"  placeholder="min" />
								</div>
								<div class="col-xs-6">
									<input type="text" name="max_share" class="form-control" value="<?php if(!empty($_GET['max_share'])){ echo $_GET['max_share']; }  ?>"  placeholder="max" />
								</div>
							</div>
							...
							<p class="mt-20">
							<a href="<?php echo base_url('investor/profiles'); ?>" class="btn btn-info mb-0 mr-10">Clear</a>
						...
						<?php 
							...
							<div class="col-sm-6 col-md-4">
								...
								?>				
									...
									<div class="mb-10">
										<div class="gray-text">Association Role
</div>
										<div class="dark-text">
			<?php $locations_decode = array_filter(json_decode($p['association'])); 
	foreach($locations_decode as $key=>$val){
		echo $skills[$val]['skill'].',&nbsp'; } ?></div>
									</div>
									<div class="mb-10">
								<div class="gray-text">Approx Investment</div>
					<div class="dark-text">
					<?php echo $p['investment_currency']; ?> <?php echo $p['min_invest']; ?>-<?php echo $p['max_invest']; ?></div>							
									</div>									
									<div class="mb-10">
									<div class="gray-text">Approx Share</div>
									<div class="dark-text"><?php echo $p['share_currency']; ?> <?php echo $p['min_share']; ?>-<?php echo $p['min_share']; ?></div>
									</div>
									</div>
									</div>
									<div>
